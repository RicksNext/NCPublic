...
function positiveOrNegative(number1, number2, id) {
    if (isNaN(number2)) number2 = number1;
    if (number1 == null || number1 == undefined) number1 == number2;
...
!userInURL ? user = "bsky.app" : user = userInURL;
...
function loadDataFirstTime() {
    $.ajax({
        url: `https://api-v2.nextcounts.com/api/bluesky/user/${user}`,
        type: "GET",
        dataType: "JSON",
        success: function (data) {
            if (data.error) {
                ...
                $('#openExternalBtn')[0].href = `https://bsky.app/profile/${user}/`;
                ...
                $('head').find('title')[0].text = `Live BlueSky Follower Count for ${data.username}`;
                $("#userbrand-navbar")[0].innerHTML = `<a class="navbar-brand"><img class="rounded-circle img-fluid" id="userimg-header" src="${/*"https://cors-anywhere-1.ricksnext.repl.co/" + */data.avatar/*.replace("https://", "")*/}" style="height: 50px;margin-right: 5px;" /> ${data.username}</a>`
                ...
                setInterval(function () {
                    $.ajax({
                        url: `https://api-v2.nextcounts.com/api/bluesky/user/${data.handle}`,
                        type: "GET",
                        dataType: "JSON",
                        success: function (dataa) {
                            ...
                            updateCounts.blaze(dataa.posts);
                            ...
                            $(`#tweetsToday`)[0].outerHTML = positiveOrNegative(dataa.posts, oldViews, "tweetsToday");
                            
                            if (!firstLive[0] || !firstLive[1]) {
                                prevCount[0] = dataa.followers;
                                firstLive[0] = true;
                                prevCount[1] = dataa.posts;
                                firstLive[1] = true;
                            } else {
                                rates.add(0, dataa.followers - prevCount[0]);
                                rates.add(1, dataa.posts - prevCount[1]);
                                prevCount[0] = dataa.followers;
                                prevCount[1] = dataa.posts;
    ...
    $.ajax(`https://api-v2.nextcounts.com/api/stats/blueskyuser/${user}`)
        .done(function (stats) {
            ...
            oldFollowers = ndata.followers ? ndata.followers[ndata.followers.length - 1][1] : 0;
            oldViews = ndata.posts ? ndata.posts[ndata.posts.length - 1][1] : 0;
            ...
            if (ndata.followers.length > 30) {
                for (let i = 0; i < 30; i++) {
                    ...
                    $('#tableBody').append(`<tr>
                        ...
                        <td>${(ndata.posts[ndata.posts.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.posts[ndata.posts.length - (i + 1)][1], ndata.posts[ndata.posts.length - (i + 2)][1], false)}</td>
                    </tr>`);
                }
            } else {
                for (let i = 0; i < ndata.followers.length; i++) {
                    ...
                    if (ndata.followers.length - (i + 1) == 0) {
                        $('#tableBody').append(`<tr>
                            ...
                            <td>${(ndata.posts[ndata.posts.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.posts[ndata.posts.length - (i + 1)][1], ndata.posts[ndata.posts.length - (i + 1)][1], false)}</td>
                        </tr>`);
                    } else {
                        $('#tableBody').append(`<tr>
                            <td>${new Date(ndata.followers[ndata.followers.length - (i + 1)][0]).toISOString().replace('T', ' ').split('.')[0]}</td>
