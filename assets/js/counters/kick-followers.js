...
!userInURL ? user = "xqc" : user = userInURL;
...
function loadDataFirstTime() {
    $.ajax({
        url: `https://kick.com/api/v2/channels/${user}`,
        type: "GET",
        dataType: "JSON",
        success: function (dataa) {
            if(dataa.is_banned == true) {
                toastr["error"](
                    "It seems like the user you requested wasn't found or was banned. Please check if the user is correct and try again.",
                    "Uh oh..."
                );
            } else {
                $('#openExternalBtn')[0].href = `https://kick.com/${user}`;

                $('#smallEmbedBtn')[0].href = `https://nextcounts.com/embed/small/?p=kickfollowers&u=${user}`;
                $('#smallEmbedBtn-1')[0].href = `https://nextcounts.com/embed/small/?p=kickviewers&u=${user}`;

                $('#largeEmbedBtn')[0].href = `https://nextcounts.com/embed/large/?p=kickfollowers&u=${user}`;
                $('#largeEmbedBtn-1')[0].href = `https://nextcounts.com/embed/large/?p=kickviewers&u=${user}`;

                let samplePhrase = `NextCounts Live Kick Follower Counts for ${dataa.user.username}!`;
                ...
                if (dataa.verified == true) {
                    updateCounts.name(`${dataa.user.username} <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none"><path d="M9 12L11 14L15 10M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>`);
                } else {
                    updateCounts.name(dataa.user.username);
                }

                $('head').find('title')[0].text = `Live Kick Follower Count for ${dataa.user.username}`;
                $("#userbrand-navbar")[0].innerHTML = `<a class="navbar-brand"><img class="rounded-circle img-fluid" id="userimg-header" src="${dataa.user.profile_pic}" style="height: 50px;margin-right: 5px;" /> ${dataa.user.username} (@${user})</a>`
                updateCounts.pfp(dataa.user.profile_pic);
                updateCounts.banner(dataa.banner_image ? dataa.banner_image.url : "hide");
                hasBanner = true;

                new Odometer({
                    el: document.getElementById("mainOdometer"),
                    value: dataa.followers_count,
                    format: '(,ddd).dd',
                });

                new Odometer({
                    el: document.getElementById("goalOdo"),
                    value: dataa.followers_count / 2,
                    format: '(,ddd).dd',
                });
                new Odometer({
                    el: document.getElementById("secondSmallOdo"),
                    value: dataa.livestream == null ? 0 : dataa.livestream.viewer_count,
                    format: '(,ddd).dd',
                });

                updateCounts.third(dataa.livestream == null ? "---" : dataa.livestream.categories[0].name);
                ...
                setInterval(function () {
                    $.ajax({
                        ...
                        success: function (data) {
                            updateCounts.mainCount(data.followers_count);
                            updateCounts.goalCount(data.followers_count);
                            
                            if(data.livestream != null) {
                                $('#liveSpinner').show();
                                updateCounts.second(data.livestream.viewer_count);
                                updateCounts.third(data.livestream.categories[0].name);
                            } else {
                                $('#liveSpinner').hide();
                                return;
                            }
                            ...
                            if (!firstLive[0] || !firstLive[1]) {
                                prevCount[0] = data.followers_count;
                                firstLive[0] = true;
                            } else {
                                rates.add(0, data.followers_count - prevCount[0]);
                                prevCount[0] = data.followers_count;
