...
!userInURL ? user = "f0rsaken" : user = userInURL;
...
function loadDataFirstTime() {
    $.ajax({
        url: `https://api-v2.nextcounts.com/api/loco/channel/${user}`,
        type: "GET",
        dataType: "JSON",
        success: function (data) {
            if (data.error || data.success == false) {
                ...
                var samplePhrase = `NextCounts Live Loco Follower Count for ${data.username}!`;
                $('head').find('title')[0].text = `Live Loco Follower Count for ${data.username}`;
                ...
                $('#openExternalBtn')[0].href = `https://loco.com/streamers/${user}`;
                ...
                setInterval(function () {
                    $.ajax({
                        ...
                        success: function (dataa) {
                            ...
                            updateCounts.viewers(dataa.currentLiveViewers);
                            updateCounts.playingnow(dataa.currentLiveGame);
                            updateCounts.views(dataa.viewsCount);
...
var updateCounts = {
    ...
    banner: function (url) {
        if (url == "hide" || url == null || url == "" || !url) {
            document.getElementById("userBanner").style.opacity = `0`;
            document.getElementById("userImg").style.marginTop = `-20px`;
        } else {
