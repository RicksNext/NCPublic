    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error) {
                ...
                var samplePhrase = `NextCounts Live Parler Follower Count for ${data.nickname}!`;
                $('head').find('title')[0].text = `Live Parler Follower Count for ${data.nickname}`;
                $("#userbrand-navbar")[0].innerHTML = `<a class="navbar-brand"><img class="rounded-circle img-fluid" id="userimg-header" src="${data.avatar}" style="height: 50px;margin-right: 5px;" /> ${data.nickname} (@${user})</a>`;

                if (data.verified == true) {
                    updateCounts.name(`${data.nickname} <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none"><path d="M9 12L11 14L15 10M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>`);
                } else {
                    updateCounts.name(data.nickname);
                }
                $('#openExternalBtn')[0].href = `https://parler.com/${user}`;

                $('#smallEmbedBtn')[0].href = `https://nextcounts.com/embed/small/?p=parleruser&u=${user}`;
                $('#largeEmbedBtn')[0].href = `https://nextcounts.com/embed/large/?p=parleruser&u=${user}`;
                ...
                setInterval(function () {
                    $.ajax({
                        ...
                        success: function (dataa) {
                            updateCounts.mainCount(dataa.followersCount);
                            updateCounts.following(dataa.followingCount);
                            updateCounts.goalCount(dataa.followersCount);

                            $(`#followersToday`)[0].outerHTML = positiveOrNegative(dataa.followersCount, oldFollowers, "followersToday");

                            $(`#likesToday`)[0].outerHTML = positiveOrNegative(dataa.followingCount, oldLikes, "likesToday");
                            
                            if (!firstLive[0] || !firstLive[1]) {
                                prevCount[0] = dataa.followersCount;
                                firstLive[0] = true;
                                prevCount[1] = dataa.followingCount;
                                firstLive[1] = true;
                            } else {
                                rates.add(0, dataa.followersCount - prevCount[0]);
                                rates.add(1, dataa.followingCount - prevCount[1]);
                                prevCount[0] = dataa.followersCount;
                                prevCount[1] = dataa.followingCount;
