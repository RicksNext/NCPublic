...
!userInURL ? user = "_c276782" : user = userInURL;
...
function loadDataFirstTime() {
    $.ajax({
        ...
        success: function (data) {
            if (data.error) {
                ...
                $('head').find('title')[0].text = `Live Rumble Follower Count for ${data.nickname}`;
                ...
                updateCounts.banner(data.banner || "hide");
                hasBanner = true;
                ...
                new Odometer({
                    ...
                    value: !isNaN(data.followersCount) ? data.followersCount / 2 : 0,
                    format: '(,ddd).dd',
                });
                ...
                setInterval(function () {
                    $.ajax({
                        ...
                        success: function (dataa) {
                            ...
                            updateCounts.blaze(dataa.videos);
                            ...
                            $(`#tweetsToday`)[0].outerHTML = positiveOrNegative(dataa.videos, oldViews, "tweetsToday");
    ...
    $.ajax(`https://api-v2.nextcounts.com/api/stats/rumbleuser/${user}`)
        .done(function (ndata) {
            ...
            oldViews = ndata.videos ? ndata.videos[ndata.videos.length - 1][1] : 0;
            ...
            if (ndata.followers.length > 30) {
                for (let i = 0; i < 30; i++) {
                    ...
                    $('#tableBody').append(`<tr>
                        ...
                        <td>${(ndata.videos[ndata.videos.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.videos[ndata.videos.length - (i + 1)][1], ndata.videos[ndata.videos.length - (i + 2)][1], false)}</td>
                    </tr>`);
                }
            } else {
                for (let i = 0; i < ndata.followers.length; i++) {
                    ...
                    if (ndata.followers.length - (i + 1) == 0) {
                        $('#tableBody').append(`<tr>
                            ...
                            <td>${(ndata.videos[ndata.videos.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.videos[ndata.videos.length - (i + 1)][1], ndata.videos[ndata.videos.length - (i + 1)][1], false)}</td>
                        </tr>`);
                    } else {
                        $('#tableBody').append(`<tr>
                            <td>${new Date(ndata.followers[ndata.followers.length - (i + 1)][0]).toISOString().replace('T', ' ').split('.')[0]}</td>
