...
!userInURL ? user = "25025320" : user = userInURL;
...
function loadDataFirstTime() {
    $.ajax({
        url: `https://api-v2.nextcounts.com/api/threads/user/${user}`,
        type: "GET",
        dataType: "JSON",
        success: function (data) {
            if (data.error) {
                ...
                var samplePhrase = `NextCounts Live Threads Follower Count for ${data.nickname}!`;
                $('head').find('title')[0].text = `Live Threads Follower Count for ${data.nickname}`;
                $("#userbrand-navbar")[0].innerHTML = `<a class="navbar-brand"><img class="rounded-circle img-fluid" id="userimg-header" src="${("https://mixerno.space/cors/" + (data.avatar).replace("https%3A%2F%2F", "").replaceAll("/", "%2F").replaceAll("?", "%3F").replaceAll("=", "%3D").replaceAll("&", "%26"))}" style="height: 50px;margin-right: 5px;" /> ${data.nickname} (@${data.username})</a>`;
