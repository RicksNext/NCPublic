...
function loadDataFirstTime() {
    $.ajax({
        ...
        success: function (data) {
            if (data.error) {
                ...
                $('#openExternalBtn')[0].href = `https://tiktok.com/@${user}`;
    ...
    $.ajax(`https://statsapi.nextcounts.com/tiktokuser/${user}`)
        .done(function (stats) {
            ...
            oldFollowers = ndata.followers[ndata.followers.length - 1][1], oldLikes = ndata.hearts[ndata.hearts.length - 1][1];
            ...
            if (ndata.followers.length > 30) {
                for (let i = 0; i < 30; i++) {
                    ...
                    $('#tableBody').append(`<tr>
                        ...
                        <td>${(ndata.hearts[ndata.hearts.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.hearts[ndata.hearts.length - (i + 1)][1], ndata.hearts[ndata.hearts.length - (i + 2)][1], false)}</td>
                    </tr>`);
                }
            } else {
                for (let i = 0; i < ndata.followers.length; i++) {
                    ...
                    if (ndata.followers.length - (i + 1) == 0) {
                        $('#tableBody').append(`<tr>
                            ...
                            <td>${(ndata.following[ndata.followers.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.following[ndata.followers.length - (i + 1)][1], ndata.following[ndata.followers.length - (i + 1)][1], false)}</td>
                            ...
                            <td>${(ndata.hearts[ndata.hearts.length - (i + 1)][1]).toLocaleString()} ${higherLowerOrEqual(ndata.hearts[ndata.hearts.length - (i + 1)][1], ndata.hearts[ndata.hearts.length - (i + 1)][1], false)}</td>
                        </tr>`);
                    } else {
                        $('#tableBody').append(`<tr>
                            <td>${new Date(ndata.followers[ndata.followers.length - (i + 1)][0]).toISOString().replace('T', ' ').split('.')[0]}</td>
