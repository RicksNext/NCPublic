...
if (!userInURL) {
    user = "idyjhe3pzIVZaJD85AW7Duvh2Jq2";
} else {
    user = userInURL;
}
...
function searchForUser() {
    ...
    if(searchTerm.includes("https://") || searchTerm.includes("http://")) {
        if(searchTerm.includes("storyfire.com/user/")) {
			isChannelId = true;
	...
	if(isChannelId == true) {
		$.ajax({
			url: `https://api.nextcounts.com/api/storyfire/user/${username}`,
			type: "GET",
			dataType: "JSON",
			success: function(data){
				if(data.error) {
					...
					searchBottomtext.innerHTML = `${(data.followers).toLocaleString()} Subscribers`;
		...
		$.ajax({
			url: `https://api.nextcounts.com/api/storyfire/search/user/${username}`,
			type: "GET",
			dataType: "JSON",
			success: function(data){
				if(data.error) {
					...
					searchPfp.src = data.userImg;
					searchBottomtext.innerHTML = `${(data.followers).toLocaleString()} Subscribers`;
					searchUsername.title = data.channelID;
					searchUserBox.style.cursor = `pointer`;
				}
			},
			error: function(){
...
$('head').find('title')[0].text = `Live Storyfire Subscriber Count for "${data.guild.serverName}"`;
function loadDataFirstTime() {
    
    $.ajax({
        url: `https://api.nextcounts.com/api/storyfire/user/${user}`,
        type: "GET",
        dataType: "JSON",
        success: function(data){
            if(data.error) {
...
var updateCounts = {
    ...
    blaze: function(count) {
        document.getElementById('blazeOdo').innerHTML = count;
    },
